...
def scrape_url_list(urls, sheet, headers):
    ...
    for start_index in range(0, len(urls), 40):
        ...
        for url in urls[start_index:start_index + 10]:  # The first item is a list containing all the information
            output_range = "A" + str(row) + ":F" + str(row)
            ...
            if url:
                ...
                batch_list.append(
                    {'range': output_range, 'values': [[title, da_price, da_desc, da_pic, da_upc_text, da_stock_text]]})
            row += 1
        da_total_list = sheet.batch_update(batch_list)
